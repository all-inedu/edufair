...
<div class="card">
    ...
    <div class="row no-gutters">';
        ...
        if(count($uniInfo['uni_detail']) < 2) {
            $disabled="style='background:#c6c6c6 !important;border: 1px solid #c6c6c6 !important'" ; $html .='
                                <div class="col">
                                    <button class="btn btn-success btn-block btn-sm btn-book" data-toggle="modal" '
            .$disabled.' data-target="#day2">Day 2</button>
    </div>';
    }
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">';
                    $duration = 15;
                    $count = 0;
                    for($i = 1; $i <= ($time*60)/$duration; $i++){ $startTime=strtotime("+".$duration*$count." minutes",
                        strtotime($assigned_time)); $endTime=strtotime("+".$duration*$i." minutes",
                        strtotime($assigned_time)); $html
                        .='
                                        	<div class="row mb-2">
                                                <div class="col-9 pr-0">
                                                    <button class="btn btn-outline-info btn-disabled btn-block" disabled>' .date("h:i", $startTime)."
                        - ".date(" h:i", $endTime).' WIB</button>
                </div>
                <div class="col-3">
                    <button class="btn btn-primary btn-block btn-book-consul" data-uniid="'.$uniInfo[" uni_id"].'"
                        data-starttime="'.$start[0]." ".date(" h:i:s", $startTime).'"
                        data-endtime="'.$start[0]." ".date(" h:i:s", $endTime).'">Book</button>
                </div>
            </div>';
            $count++;
            }
